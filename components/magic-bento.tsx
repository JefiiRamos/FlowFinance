...
const GlobalSpotlight = ({
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      const { proximity: prox, fadeDistance: fade } = calculateSpotlightValues(spotlightRadius)

      const targetOpacity =
        minDistance <= prox
          ? 0.8
          : minDistance <= fade
            ? ((fade - minDistance) / (fade - prox)) * 0.8
            : 0
...
export const MagicBento = ({
  ...
  const cardsToRender = items ?? defaultCardData
