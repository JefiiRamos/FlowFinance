...
export function LightPillar({
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (rendererRef.current) {
        ...
        // @ts-expect-error - forceContextLoss exists in WebGLRenderer
  ...
  if (!webGLSupported) {
    return (
      <div
        className={cn(
          'absolute inset-0 flex h-full w-full items-center justify-center bg-black/10 text-sm text-muted-foreground',
          className,
        )}
        style={{ mixBlendMode }}
      >
